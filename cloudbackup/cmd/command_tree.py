...
def register_cb_command(server, config, start_backup, stop_backup, list_backups, status_backup, query_backup_records, config_setter, continue_cmd, abort_cmd):
    ...
    tree = (
        ...
        .then(Literal('start').runs(lambda src, ctx: start_backup(src, config)))
        .then(Literal('stop').runs(lambda src, ctx: stop_backup(src)))
        .then(Literal('list').runs(lambda src, ctx: list_backups(src, config)))
        .then(Literal('status').runs(lambda src, ctx: status_backup(src)))
        .then(Literal('history').runs(lambda src, ctx: query_backup_records(src, 5)))
        .then(config_branch)
